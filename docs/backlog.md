...
| V2-02 | P2 | todo | jesse | Schedule memory_maintenance.py as Krypton job | `scripts/memory_maintenance.py --auto-archive` handles staleness sweep, duplicate detection, unscoped check, and reconcile. Needs to run on a schedule via Krypton (recommended: noon + 8pm to bookend the workday). Script path on dev VM: `/mnt/mac/Users/jessepike/code/_shared/memory/scripts/memory_maintenance.py`. | Job runs on schedule on Krypton, maintenance report is accessible, zero manual intervention needed. |
| V2-03 | P3 | todo | agent | Type rebalancing: update agent prompts to capture decisions and preferences | 85% of memories are observations, 13% decisions, <1% preferences. Decisions/preferences are higher-signal for guiding future behavior. Each agent's write_memory instructions need to explicitly prompt for decision capture (choices between alternatives) and preference capture (user corrections/approvals). Agents to update: Krypton GM/EOD, Codex AGENTS.md, Claude Code session protocol, CxO agents. | Type distribution shifts toward >=25% decisions, >=5% preferences over a 30-day measurement window. |
| V2-04 | P3 | todo | agent | LLM-based consolidation for the 0.75-0.88 similarity zone | Current dedup is binary (ADD/SKIP at 0.88 threshold). Memories in the 0.75-0.88 similarity range are related but not identical — these need LLM-assisted consolidation. **Design decisions captured:** (1) Deferred batch, not inline — add consolidation_sweep step to maintenance script, run at noon + 8pm with maintenance job. (2) Actions: ADD (keep both), SKIP (discard new), UPDATE (new supersedes old), MERGE (LLM combines into single entry, archive originals). (3) LLM: TBD — API call from Krypton (breaks offline constraint) vs small local model on Mac mini. (4) Timing: revisit after ~2 more months of corpus growth (~June 2026) when there's enough data to know whether the gap is mostly UPDATE or MERGE cases. | Consolidation sweep runs as part of maintenance batch. Memories in 0.75-0.88 zone are processed as UPDATE/MERGE/ADD. Zero manual duplicate cleanup needed. |
