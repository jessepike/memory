...
|----|----------|--------|-------|-------------|-----------|
| DEV-12 | P1 | in_progress | agent | MCP integration polish and end-to-end smoke testing | MCP tools execute against real storage with passing smoke test checklist |

## Todo

| id | priority | status | owner | description | done_when |
|----|----------|--------|-------|-------------|-----------|
| DEV-13 | P1 | todo | agent | Harden scope authorization rules for update/archive defense-in-depth namespace checks | Non-privileged caller path covered by tests and enforced in API |
| DEV-14 | P1 | todo | agent | Add MCP-level integration tests for 9-tool surface and error contracts | Integration suite verifies tool payloads, scope errors, and lifecycle flows |
